...
--SELECT * FROM FAQ;
INSERT INTO FAQ VALUES('구매상당만 가능합니다','우리는 구매상담신청을 받아 딜러와 연결하여 상담만 가능합니다. 따라서 구매가 가능하지 않으니 구매 상담신청시 이점 참고하시기 바랍니다.');
...
SELECT
    * FROM ev JOIN (
    SELECT modelname, count(modelname) "COUNSELING" FROM buy_counseling GROUP by modelname
    ) bc on ev.modelname = bc.modelname;
...
-- INSERT INTO MEMBER VALUES(SEQ_UNO.NEXTVAL, ?, ?, DEFAULT, ?, ?, DEFAULT);
-- UPDATE MEMBER SET NAME = ?, PHONE = ? WHERE USER_NO = ?;
...
COMMIT;
INSERT INTO BOARD VALUES(SEQ_BOARD_NO.NEXTVAL, 'test', '게시글 제목AAAAA' ||SEQ_BOARD_NO.CURRVAL, '게시글 내용' ||SEQ_BOARD_NO.CURRVAL, NULL, NULL, DEFAULT, SYSDATE, SYSDATE);
SELECT * FROM BOARD;
